...
if(isset($_GET['del']))
{
    ...
    if(!empty($del))
    {
        $delete = mysqli_query($con, "delete from plans where id='$del'");
        if($delete)
        {
                    ...
                    <div class="col-lg-12 col-md-12">
                        <div class="card custom-card">
                            <div class="card-body p-0">
                                <?php
                                $result = mysqli_query($con, "select plans.id, plans.plan_name,zones.zone_name,plans.GB,plans.Mins,plans.SMS,plans.Days,plans.tags,plans.USD,plans.content,plans.date,CASE WHEN plans.status='A' THEN 'Active' ELSE 'Deactive' END AS status from plans INNER JOIN zones ON plans.zone_id=zones.id");          
                                $i = 1;
                                $rows =  '[';
...
<span style="float:right;" class="row">
    <div class="col-md-6">
        <div class="totalCnts text-center">
            Total Continents : <span class="boldLt" id="filRecTot"><?php echo 0; ?></span> |
    ...
    <div class="col-md-3">
        <div class="text-right">
            <a href="add-plan.php" class="btn btn-primary btn-rounded py-1"><i class="fa fa-plus"></i></a>
...
$(document).ready(function () {
    ...
    $treeGrid.jqxGrid(
    {
        ...
        columns: [
            { text: 'Plan ID', dataField: 'id', width: 60, cellsalign: 'center', align: 'center' },
            { text: 'Plan Name', dataField: 'plan_name', width:200},
            { text: 'Zone', datafield: 'zone_name', filtertype: 'checkedlist', width: 100, cellsalign: 'center', align: 'center' },
            { text: 'GB', datafield: 'GB', filtertype: 'checkedlist', width: 100, cellsalign: 'center', align: 'center' },
            { text: 'Mins', datafield: 'Mins', filterable:false, width: 100, cellsalign: 'center', align: 'center' },
            { text: 'SMS', datafield: 'SMS', filtertype: 'checkedlist', width: 100, cellsalign: 'center', align: 'center' },
            { text: 'Days', datafield: 'Days', filtertype: 'checkedlist', width: 100, cellsalign: 'center', align: 'center' },
            { text: 'Content', datafield: 'content', filterable:false, width: 150, cellsalign: 'center', align: 'center' },
            { text: 'USD', datafield: 'USD', width: 100, cellsalign: 'center', align: 'center' },
            { text: 'Tags', datafield: 'tags', width: 150, cellsalign: 'center', align: 'center' },
            { text: 'Status', datafield: 'status', filtertype: 'checkedlist', width: 100, cellsalign: 'center', align: 'center' },
            { text: 'Creation Date', datafield: 'date', cellsformat:'dd-MMM-yy HH:mm:ss (ddd)', filtertype: 'date', width: 200, cellsalign: 'center' },
            { text: 'Action', datafield: 'action',filterable:false, width: 100, cellsalign: 'center', align: 'center',
                cellsrenderer: function (row) {
                   
                    return '<div class="text-center my-2"><a href="edit-plan.php?id='+ $treeGrid.jqxGrid('getCellValue', row, 'id') +'"><i class="fa fa-pen mr-2" aria-hidden="true" title="Edit"></i></a> <a href="delete-plan.php?del='+ $treeGrid.jqxGrid('getCellValue', row, 'id') +'"><i class="fa fa-trash text-danger" aria-hidden="true" title="Delete"></i></a></div>';
                }
            },
        ]
    });
