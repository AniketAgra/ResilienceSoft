...
$id     = mysqli_real_escape_string($con,$_POST['brandid']);
...
{
    $update = mysqli_query($con,"UPDATE brands SET brandName='$name', vendorId='$vendor', notes='$note', status='$status', modifyBy='$userid', modifyAt='$date' where id='$id'");
    if($update)
    {
        echo '<script>$("#brand-form")[0].reset();</script>';
        echo '<script>$("#error").html("Brand Successully Update");$("#error").attr("style", "color: #3ec845 !important");</script>';
    }
}
?>    
