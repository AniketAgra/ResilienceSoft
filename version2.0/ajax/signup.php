...
else
{
	...
	if(mysqli_num_rows($check)===0)
	{
		...
		if($insert)
		{
			...
			$url = constant("SITEURL")."verification?userId=".$userId.'&ecode='.$emailcode;
			$url1 = constant("SITEURL")."verification";
	
			$message = '<table cellspacing="0" cellpadding="0" style="width: 540px; font-family: calibri; letter-spacing: normal; text-indent: 0px; text-transform: none; word-spacing: 0px; text-decoration: none;">
				    <tbody>
				        <tr>
				            <td>
				                <table cellspacing="0" cellpadding="0" style="width: 500px; margin: 0px 20px;">
				                    <tbody>
				                        ...
				                        <tr>
				                            <td style="padding-left: 0px; padding-top: 9px; padding-bottom: 9px;font-size: 15px; font-family: calibri">
				                                <div style="margin: 0px;font-size: 15px;">Dear '.$fname.' '.$lname.',</div>
				                                <div style="margin: 0px;font-size: 15px;">Per your request, here is your gsm2go eSIM verification code (one time password): </div>
				                                <div style="margin: 0px;font-size: 15px;">'.$emailcode.'</div>
				                                <div style="margin: 0px;font-size: 15px;">Please enter the code to confirm your registration.</div>
				                                <div style="margin: 0px;font-size: 15px;">Thank you from <span style="color:rgb(0,112,192)">gsm</span><i><span style="color:rgb(0,176,80)">2go</span></i></div>
				                            </td>
				                        </tr>
				                    </tbody>
				                </table>
