                        ...
                        <div class="col-md-3">
                            ...
                            <div class="card card-blog-entry border-0">
                                <div class="hover-item rounded-ultra shadow-mdz">
                                    <a href="#" data-toggle="modal" data-target="#modalZones<?php echo $row['id']; ?>">
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <ul class="nav nav-pills" id="pills-tab" role="tablist">
                                            ...
                                            ?>
                                            <li class="nav-item">
                                                <a class="nav-link <?php if($i == 1){ echo 'active'; } ?>" id="pills-first-tab<?php echo $plan_row['id']; ?>"zz data-toggle="pill" href="#pills-first_<?php echo $plan_row['id']; ?>" role="tab" aria-controls="pills-first" aria-selected="false"><?php echo $plan_row['Days']; ?> Days</a>
                                            </li>
                                            <?php $i++; } ?>
                                        </ul>
                                                ...
                                                <div class="row">
                                                    <form method="post">
                                                        ...
                                                        <div class="col-12 col-md-6 mx-auto">
                                                            ...
                                                            $cart_details = mysqli_query($con, "select * from cart_items where cookie_id='".$cookieValue."' AND plan_id='".$plan_row['id']."'");
                                                            if(mysqli_num_rows($cart_details) > 0) { ?>
                                                                <button type="button" onclick="window.location='cart2'; return false;" id="addtocart_<?php echo $plan_row['id']; ?>" style="font-size:15px;" class="btn btn-block btn-round btn-md btn-success mx-2 mt-4 mx-lg-3 mb-4 buy-now<?php echo $plan_row['id']; ?>" disabled>Added to cart</button>
                                                            <?php } else { ?>
                                                                <button type="button" onclick="add2cart('<?php echo $plan_row['id']; ?>');" id="addtocart_<?php echo $plan_row['id']; ?>" style="font-size:15px;" class="btn btn-block btn-round btn-md btn-success mx-2 mt-4 mx-lg-3 mb-4 buy-now<?php echo $plan_row['id']; ?>" disabled>Add to cart</button>
                                                            <?php } ?>
                                                            <?php if(isset($_SESSION['user'],$_SESSION['role']) && $_SESSION['role']=="U"){ ?>
                                                            <button type="submit" formaction="<?php echo constant("SITEURL"); ?>checkout" id="<?php echo $plan_row['id']; ?>" style="font-size:15px;" class="btn btn-block btn-round btn-md btn-primary mx-2 mt-4 mx-lg-3 mb-4 buy-now<?php echo $plan_row['id']; ?>" disabled>Proceed to Buy</button>
                                                            <?php } else{ ?>
                                                            <button type="submit" formaction="<?php echo constant("SITEURL"); ?>signup" id="<?php echo $plan_row['id']; ?>" style="font-size:15px;" class="btn btn-block btn-round btn-md btn-primary mx-2 mt-4 mx-lg-3 mb-4 buy-now<?php echo $plan_row['id']; ?>" disabled>Proceed to Buy</button>
                                                            <?php } ?>
                                                        </div>
                                                    </form>
                                                </div>
...
<script>
    function add2cart(planId) {
        ...
        $.ajax({
            ...
            type: "POST",
            data: { planId:planId,esimLive:esimLive },
